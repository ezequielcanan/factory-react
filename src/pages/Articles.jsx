...
const Articles = ({materials = false}) => {
  ...
  return (
    <Main className={"grid gap-6 items-start"}>
      <section className="grid items-center justify-center gap-8 md:items-start md:grid-cols-2 md:justify-between">
        <Title text={!materials ? "Articulos" : "Insumos"} className={"text-center md:text-start"}/>
        <div className="flex items-center gap-4 md:justify-end justify-center flex-wrap">
          ...
          <Link to={`/${!materials ? "articles" : "materials"}/new`} className="justify-self-end"><Button className={"text-xl font-bold px-4 flex gap-x-4 items-center"}>Agregar {!materials ? "articulo" : "insumo"} <FaPlus/></Button></Link>
        </div>
      </section>
      <ArticlesContainer containerClassName={"w-full"} societyState={society} setSocietyState={setSociety} materials={materials}/>
    </Main>
  )
}
